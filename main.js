...
class FirstPersonCameraDemo {
  ...
  initializeAudio_() {
    ...
    loader.load('resources/music/Phoenix-FataVerde.mp3', (buffer) => {
      setTimeout(() => {
    ...
    loader.load('resources/music/TrecetiBatalioaneRomaneCarpatii.mp3', (buffer) => {
      setTimeout(() => {
  ...
  initializeScene_() {
    ...
    const texture = loader.load([        './resources/skybox/posx.jpg',
  ...
  loadAnimatedModelAndPlay_(path, modelFile, animFile) {
  ...
  _LoadAnimatedModel() {
    const anim = new FBXLoader();
    anim.load('./resources/animation/ArmsHipHopDance_BigGuy.fbx', (anim) => {
      this._mixer = new THREE1.AnimationMixer(fbx);
      const idle = this._mixer.clipAction(anim.animation[0]);
      idle.play();
    });
    this
  }
